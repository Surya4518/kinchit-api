...
class IndexController extends Controller
{
    public function UserRegister(Request $request)
    {
        try {
            ...
            $rules = [
                ...
                'your_acharyan' => 'required|string|max:255',
                'your_mobile' => 'required|max:255|regex:/^[0-9]{10}$/|unique:userprofile_dt,phone_number',
                'your_whatsapp' => 'required|max:255|regex:/^[0-9]{10}$/|unique:userprofile_dt,phone_number_wa',
            ...
            Mail::to($request->your_email)->send(new MyMail(null , $arr,null,null,null));
    ...
    public function UserLogin(Request $request)
    {
        try {
            ...
            $user_cnt = User::where('user_login', '=', $request->username)->where('user_pass', '=', md5($request->password))->where('user_status', '=', '0')->whereNotNull('verified_at')->count();
            $user_cnt1 = User::where('user_email', '=', $request->username)->where('user_pass', '=', md5($request->password))->where('user_status', '=', '0')->whereNotNull('verified_at')->count();
            if($user_cnt > 0){
                $user = User::where('user_login', '=', $request->username)->where('user_pass', '=', md5($request->password))->where('user_status', '=', '0')->whereNotNull('verified_at')->get();
            }else if($user_cnt1 > 0){
                $user = User::where('user_email', '=', $request->username)->where('user_pass', '=', md5($request->password))->where('user_status', '=', '0')->whereNotNull('verified_at')->get();
            }else{
                $user = User::where('user_login', '=', $request->username)->where('user_pass', '=', md5($request->password))->where('user_status', '=', '0')->whereNotNull('verified_at')->get();
            }
            if ($user->count() > 0) {
                ...
                $user_update = User::where('user_login', '=', $request->username)->where('user_pass', '=', md5($request->password))->where('user_status', '=', '0')->whereNotNull('verified_at')->update(['web_token' => $token]);
                $user123 = User::where('web_token', '=', $token)->get();
            }elseif($request->device_id == '2'){
                $user_update = User::where('user_login', '=', $request->username)->where('user_pass', '=', md5($request->password))->where('user_status', '=', '0')->whereNotNull('verified_at')->update(['app_token' => $token,'fcm_token' => $request->fcm_token]);
            ...
            return response()->json($user->count() > 0 ? ['status' => 200, 'message' => 'Successfully logged in.', 'user_type' => $user123[0]->user_type, 'user_token' => $token,'marital_acc' => $matri_details->count()] : ['status' => 400, 'message' => 'Invalid credentials.']);
        } catch (Exception $e) {
            return response()->json([
                'status' => 500,
                'error' => $e->getMessage(),
           ...
           $userdata =  User::where('remember_token',  $request->remember_token)->first();
             if($userdata){
                $updatepassword = [
                 ...
                 Mail::to($userdata->user_email)->send(new MyMail(null,null,null,null,$updatepassword,null,null,null,null,null,null,null,null,null));
             }
           ...
           $userdata =  User::where('ID', $ses_user[0]->ID)->first();
             if($userdata){
                $updatepassword = [
                 ...
                 Mail::to($userdata->user_email)->send(new MyMail(null,null,null,null,$updatepassword,null,null));
             }
    ...
    public function UpdateTheProfile(Request $request)
    {
        try {
            ...
            $rules = [
                ...
                'user_email' => 'required',
                'land_line' => 'required',
                'user_sms_no' => ['required', 'max:15', 'regex:/^[^+][0-9]{0,14}$/'],
                'user_wa_no' => ['required', 'max:15', 'regex:/^[^+][0-9]{0,14}$/'],
                'address1' => 'required',
                'address2' => 'required',
                'area' => 'required',
                'city_name' => 'required',
            ...
            $user_old_data = DB::table('userprofile_dt')->where('user_id', $request->user_edit_id)->get();
             $city  = DB::table('city')->where('name', $request->city_name)->first();
             $state  = DB::table('state')->where('name', $request->state_name)->first();
             $country  = DB::table('country')->where('name', $request->country_name)->first();
            ...
            $user_profile_arr = [
                ...
                'landline_ph_no' => $request->land_line,
                'user_email' => $request->user_email,
                'phone_number' => $request->user_sms_no,
                'phone_number_wa' => $request->user_wa_no,
                'address_1' => $request->address1,
    ...
    public function MyProfile(Request $request){
        try{
         ...
         $myprofile = DB::table('wpu6_users as w')
            ->select('w.ID','w.user_login','u.first_name','u.last_name','u.dob','u.gender','u.acharyan','u.user_email'
            ,'u.landline_ph_no','u.phone_number','u.phone_number_wa','u.address_1','u.address_2','u.area','u.city'
            ,'u.state' ,'u.postcode','u.country','u.kinchit_enpani','u.payment_method','co.name as CountryName','c.name as CityName','s.name as StateName','u.user_image')
            ->leftJoin('userprofile_dt as u', 'w.ID', '=', 'u.user_id')
            ->leftJoin('country as co', 'co.id', '=', 'u.country')
            ->leftJoin('city as c', 'c.id', '=', 'u.city')
            ->leftJoin('state as s', 's.id', '=', 'u.State')
            ->where('w.ID', $ses_user[0]->ID)
            ->get();
        ...
        try{
            ...
            if(!empty($user)){
                if($request->device_id == '1'){
                    ...
                    Mail::to($Email)->send(new MyMail($data,null,null,null,null,null,null,null,null,null,null,null,null,null));
                    ...
                    Mail::to($Email)->send(new MyMail($data,null,null,null,null,null,null,null,null,null,null,null,null,null));
                    $user_update = User::where('user_email', $Email)->update(['remember_token' => $token]);
                    $user_update = User::where('user_email', $Email)->update(['otp' => $otp_number]);
                }
...
// public function ForgetPassword(Request $request){
    ...
    //                 'token' =>  $token
    //             ];
    //             Mail::to($Email)->send(new MyMail($data,null,null,null,null,null,null));
    //             $user_update = User::where('user_email', $Email)->update(['remember_token' => $token]);
    ...
    public function otpVerify(Request $request){
        try{
            ...
            if (!empty($verify)) {
               ...
               $token = User::select('remember_token','app_token','otp')->where('ID', $verify->ID)->get();
                return response()->json(['status' => 200, 'message' => 'Otp Verified Successfully','data' => $token]);  
            }else{
    ...
    public function logout(Request $request)
    {
      ...
      try{
        ...
        if ($user) {
          if($request->device_id == '1'){
            User::where('ID', $ses_user[0]->ID)->update(['web_token' => null]);
          }else{
            User::where('ID', $ses_user[0]->ID)->update(['app_token' => null]);
          }
            session()->flush();
        ...
        try {
            ...
            if ($user) {
                ...
                $arr = [
                 ...
                 Mail::to($user->user_email)->send(new MyMail(null, $arr, null,null,null,null,null));
    ...
    public function GetTheUserProfile(Request $request)
    {
        try {
            $ses_user = Session::get('user');
                $user_data = User::where('ID', '=', $ses_user[0]->ID)->get();
                $matri_details = DB::table('m4winreg')->where('user_id', '=', $user_data[0]->ID)->get();
            return response()->json($user_data->count() > 0 ? ['status' => 200, 'user_type' => $user_data[0]->user_type, 'user_token' => $user_data[0]->web_token,'marital_acc' => $matri_details->count()] : ['status' => 400, 'message' => 'Invalid credentials.']);
        } catch (Exception $e) {
            return response()->json([
                'status' => 500,
                'error' => $e->getMessage(),
    ...
    public function UserLoginUpdate(Request $request){
        try {
            $user_data = User::where('app_token', '=', $request->token)->get();
            if($user_data->count() > 0){
                $user_update = User::where('app_token', '=', $request->token)->update(['web_token'=>$request->token]);
                $user_data1 = User::where('web_token', '=', $request->token)->get();
                $matri_details = DB::table('m4winreg')->where('user_id', '=', $user_data1[0]->ID)->get();
                return response()->json($user_data1->count() > 0 ? ['status' => 200, 'user_type' => $user_data[0]->user_type, 'user_token' => $user_data[0]->web_token,'marital_acc' => $matri_details->count()] : ['status' => 400, 'message' => 'Invalid credentials.']);
            }else{
